<template>
	...
	<div v-if="lesson.data" class="">
		...
		<div :class="['h-screen grid', sidebarMinimized ? '' : 'md:grid-cols-[40%,60%]']">
			<div v-show="!sidebarMinimized" class="px-5 py-3">
				<CourseSidebar
					ref="childRef"
					:courseName="courseName"
					...
					:hasEnrollment="!!(user && lesson.data.membership)"
					:hasPrev="!!lesson.data.prev"
					:hasNext="!!lesson.data.next"
					:isNextDisabled="!isLessonComplete"
					:outlineKey="chapterNumber"
					:getProgress="!!lesson.data.membership"
					@prev="switchLesson('prev')"
					@next="switchLesson('next')"
				/>
			</div>
			<div v-if="lesson.data.no_preview" class="">
...
import CustomBreadcrumbs from '@/components/ui/CustomBreadcrumb.vue'
