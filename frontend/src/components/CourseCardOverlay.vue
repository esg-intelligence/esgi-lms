	...
	<div class="bg-white rounded-xl w-full border border-gray-100 shadow-xl shadow-gray-100 p-5 lg:sticky lg:top-14">
		...
		<div class="py-4 space-y-4">
			...
			<div v-if="!readOnlyMode" class="space-y-2 !mt-20">
				...
				<Button v-if="canGetCertificate" @click="fetchCertificate()" variant="outline"
					class="w-full mt-2 hidden" size="md">
					{{ __('Claim Certificate') }}
				</Button>
				<Button v-if="certification.data && certification.data.certificate" @click="copyCertificateUrl()"
					variant="outline" class="w-full mt-2" size="md">
					<template #prefix>
