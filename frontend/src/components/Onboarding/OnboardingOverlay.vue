<template>
	<div v-if="show" class="fixed inset-0 z-[100] bg-white flex flex-col overflow-y-auto">
		<div class="flex items-center justify-between px-8 py-4 w-full border-b h-16 sticky top-0 bg-white z-20">
					...
					<div v-for="(slide, index) in slides" :key="index"
						...
						<div class="max-w-3xl mx-auto flex flex-col items-center w-full">
							<div class="mb-12 relative w-full flex items-center justify-center">
								<div v-if="slide.type === 'video' && slide.videoUrl"
									class="w-full flex items-center justify-center">
									<iframe :src="slide.videoUrl" class="aspect-video w-full" allow="autoplay"
										allowfullscreen>
									</iframe>
								</div>
								<div v-else-if="slide.type === 'image' && slide.imageUrl"
									class="w-full flex items-center justify-center">
									<img :src="slide.imageUrl" :alt="slide.title" class="w-full h-80 object-contain" />
								</div>
							</div>
...
const slides = [
	...
	},
	{
		...
		description: 'Receive instant explanations, smart recommendations, and learning support that helps you understand complex topics faster and more efficiently.',
		imageUrl: '/assets/lms/images/undraw_online-stats_d57c 1.png',
		type: 'image',
	},
	{
		title: 'Learn without limit',
		description: 'Access your courses anytime, anywhere, and progress at your own pace in a flexible environment built for modern learners.',
